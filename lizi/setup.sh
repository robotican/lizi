...
INSTALL_HW_COMPS=false #deremine if this script will install lizi hardware drivers
...
if [ "$INSTALL_HW_COMPS" = true ] ; then
    ...
    sudo cp $PKG_PATH/lizi/rules/49-teensy.rules /etc/udev/rules.d
    sudo cp $PKG_PATH/lizi/rules/hokuyo.rules /etc/udev/rules.d/
    sudo cp $PKG_PATH/lizi/rules/99-realsense-libusb.rules /etc/udev/rules.d
